...
def vista_previa_historial():
    ...
    pdf_path = get_project_root() / "PDF" / "Historial_Eliminados.pdf"
    if not pdf_path.exists():
        raise FileNotFoundError(f"El archivo {pdf_path} no existe.")
    imagen_bytes = previsualizar_pdf(pdf_path)

    root=tk.Toplevel()
    root.title("Imprimir Historial")
    root.geometry("700x650+400+40")
    image = Image.open(io.BytesIO(imagen_bytes))
    photo = ImageTk.PhotoImage(image.resize((700, 500)))
    ...
    label.image = photo
    label.pack(padx=10, pady=5)
    titulo = tk.Label(root, text="¿Estás seguro que deseas exportar el Reporte de Historial en PDF?", font=('Helvetica', 16), fg="black")
    titulo.pack(padx=10, pady=5)
    
    def open_pdf_historial(): 
        os.startfile(str(pdf_path))
    
    confirmar_boton = CTkButton(root,text="Confirmar",command=lambda: (root.destroy(), open_pdf_historial()),
                                           text_color="white",cursor='hand2',
                                        fg_color="#00501B",hover_color="#57bd9e", font=("Arial", 14,"bold"))
    confirmar_boton.pack(padx=10,pady=3)
